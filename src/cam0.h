...
public:
    ...
    void handle_poll_event(SDL_Event& event, scene& world) {
        while (SDL_PollEvent(&event)) {
            ...
            if (!ImGui::GetIO().WantCaptureMouse && !ImGui::GetIO().WantCaptureKeyboard) {
                ...
                // Right-click to activate mouse camera control
                if (event.type == SDL_MOUSEBUTTONDOWN) {
                    if (event.button.button == SDL_BUTTON_LEFT && valid(event.button.x, event.button.y)) {
                        ...
                        mouse_grabbed = true;
                        SDL_SetRelativeMouseMode(SDL_TRUE); // Lock mouse in window during camera control
                ...
                if (event.type == SDL_MOUSEBUTTONUP) {
                    ...
                    if (event.button.button == SDL_BUTTON_RIGHT) {
                        mouse_grabbed = false;
                        SDL_SetRelativeMouseMode(SDL_FALSE); // Release mouse when done with camera control
                        std::clog << "Camera released\n";
                    }
                }
                ...
                if (event.type == SDL_MOUSEMOTION) {
                    if (mouse_grabbed) {
                        ...
                        // Immediately update camera after mouse movement
                        update_camera();
                        std::clog << "Camera rotation: yaw=" << yaw << ", pitch=" << pitch << "\n";
                    }
        ...
        // Compute forward and right vectors based on current yaw and pitch
        vec3 forward(cos(pitch) * cos(yaw), sin(pitch), cos(pitch) * sin(yaw));
        vec3 right(-sin(yaw), 0, cos(yaw));
        
        if (keys[SDL_SCANCODE_W] || keys[SDL_SCANCODE_UP]) {
            lookfrom = lookfrom + forward * move_speed;
            camera_moved = true;
        }
        ...
        if (keys[SDL_SCANCODE_A] || keys[SDL_SCANCODE_LEFT]) {
            lookfrom = lookfrom - right * move_speed;
            camera_moved = true;
        }
        ...
        // Update camera if there was any movement
        if (camera_moved) {
            update_camera();
            std::clog << "Camera position: " << lookfrom.x() << ", " << lookfrom.y() << ", " << lookfrom.z() << "\n";
        }
    }
    ...
    bool render(scene& world) {
        ...
        std::vector<int> sample_counts(render_width * render_height, 0);
        SDL_Event event;
        ...
        while (running) {
            ...
            // Create UI panel for scene controls
            ...
            // Demo window for debugging UI
            // ImGui::ShowDemoWindow();

            // Render the scene with multiple threads
            ...
            for (int t = 0; t < num_threads; ++t) {
                ...
                threads.emplace_back([this, &world, &pixel_buffer, &sample_counts, start_row, end_row]() {
            ...
            // Convert rendered image to pixel data for display
            ...
            for (int j = 0; j < render_height; ++j) {
                for (int i = 0; i < render_width; ++i) {
                    int idx = j * render_width + i;
                    color c = pixel_buffer[idx] / (sample_counts[idx] > 0 ? sample_counts[idx] : 1);
...
private:
    ...
    bool initialize() {
        ...
        window = SDL_CreateWindow("Ray Tracer",
            SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, window_width + gui_width, window_height,
        ...
        // Initialize camera direction based on current yaw/pitch
        update_camera();
    ...
    void update_camera() {
        ...
        // Calculate new lookat point based on lookfrom and direction
        ...
        // Calculate corner pixel location
